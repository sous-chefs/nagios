...
if node.run_list.roles.include?(node['nagios']['server_role'])
  mon_host << (node['nagios']['monitored_client_interface'] ? node['network']["ipaddress_#{node['nagios']['monitored_client_interface']}"] : node['ipaddress'])
elsif node['nagios']['multi_environment_monitoring']
  search(:node, "role:#{node['nagios']['server_role']}") do |n|
    mon_host << (node['nagios']['monitored_client_interface'] ? n['network']["ipaddress_#{node['nagios']['monitored_client_interface']}"] : n['ipaddress'])
  end
else
