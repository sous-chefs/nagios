...
* `node['nagios']['whitelist_ips']` - the array of ip addresses that will be whitelisted if `node['munin']['server_auth_method'] == 'whitelist'`
* `node['nagios']['apply_firewall_rules']` - whether or not to add rules to the firewall based on `node['munin']['whitelist_ips']`. Default is true.
